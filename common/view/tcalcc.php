...
$sort = array('0'=>1,'_fnorder'=>1);//,'16'=>1);
$limit = 1000000;
$skip = 0;
$datestr = $argv[1];
...
/*
$c_dzd = $mon->findByIndex('dzd',(object)$cond,$limit,$skip,array(),array(0=>1,10=>1),true);
//由于不知到顺序
while($row = $c_dzd->getNext()){
}
// */
...
while($row = $c->getNext()){
    ...
    $touru = $totalr['yinhangzc'] + $totalr['yinhangzr'];
    $qse = $totalr[8];
    $qsje = $row[8];
    $r9 = $row[9];
